...
namespace LightVideoDecoder
{
  ...
  template<>void defilterSubTop<uint8_t>(ImageChannel<uint8_t> &img)
  {
    ...
    else
    {
      for(int y = 1; y < height; ++y)
      {
        int alignDiff = (16 - (reinterpret_cast<size_t>(img.data()) % 16)) % 16;
        for(int x = 0; x < alignDiff; ++x)
          img(y, x) += img(y - 1, x);
        for(int x = alignDiff; x < width - (width - alignDiff) % 16; x += 16)
        {
          __m128i a = _mm_load_si128(reinterpret_cast<__m128i*>(&img(y - 1, x)));
          __m128i b = _mm_load_si128(reinterpret_cast<__m128i*>(&img(y, x)));
          a = _mm_add_epi8(a, b);
          _mm_store_si128(reinterpret_cast<__m128i*>(&img(y, x)), a);
        }
  ...
  template<>void defilterSubLeft<uint8_t>(ImageChannel<uint8_t> &img)
  {
    ...
    else
    {
      for(int y = 0; y < height; ++y)
      {
        int alignDiff = 16 - reinterpret_cast<size_t>(&img(y, 0)) % 16;
        for(int x = 1; x < alignDiff; ++x)
          img(y, x) += img(y, x - 1);
        for(int x = alignDiff; x < width - (width - alignDiff) % 16; x += 16)
        {
          __m128i p0, s0;
          p0 = _mm_load_si128(reinterpret_cast<__m128i*>(&img(y, x)));

          s0 = _mm_slli_si128(p0, 1);
          p0 = _mm_add_epi8(p0, s0);
          s0 = _mm_slli_si128(p0, 2);
          p0 = _mm_add_epi8(p0, s0);
          s0 = _mm_slli_si128(p0, 4);
          p0 = _mm_add_epi8(p0, s0);
          s0 = _mm_slli_si128(p0, 8);
          p0 = _mm_add_epi8(p0, s0);

          s0 = _mm_set1_epi8(img(y, x - 1));
          p0 = _mm_add_epi8(p0, s0);

          _mm_store_si128(reinterpret_cast<__m128i*>(&img(y, x)), p0);
        }
        for(int x = width - (width - alignDiff) % 16; x < width; ++x)
          img(y, x) += img(y, x - 1);
      }
  ...
  template<>void defilterSubTop<uint16_t>(ImageChannel<uint16_t> &img)
  {
    ...
    else
    {
      for(int y = 1; y < height; ++y)
      {
        int alignDiff = (8 - (reinterpret_cast<size_t>(img.data()) % 16) / 2) % 8;
        for(int x = 0; x < alignDiff; ++x)
          img(y, x) += img(y - 1, x);
        for(int x = alignDiff; x < width - (width - alignDiff) % 8; x += 8)
        {
          __m128i a = _mm_load_si128(reinterpret_cast<__m128i*>(&img(y - 1, x)));
          __m128i b = _mm_load_si128(reinterpret_cast<__m128i*>(&img(y, x)));
          a = _mm_add_epi16(a, b);
        ...
        for(int x = width - (width - alignDiff) % 8; x < width; ++x)
          img(y, x) += img(y - 1, x);
      }
    }
  }
  ...
  template<>void defilterSubLeft<uint16_t>(ImageChannel<uint16_t> &img)
  {
    ...
    else
    {
      for(int y = 0; y < height; ++y)
      {
        int alignDiff = 8 - (reinterpret_cast<size_t>(&img(y, 0)) % 16) / 2;
        for(int x = 1; x < alignDiff; ++x)
          img(y, x) += img(y, x - 1);
        for(int x = alignDiff; x < width - (width - alignDiff) % 8; x += 8)
        {
          ...
          s0 = _mm_slli_si128(p0, 2);
          p0 = _mm_add_epi16(p0, s0);
          s0 = _mm_slli_si128(p0, 4);
          p0 = _mm_add_epi16(p0, s0);
          s0 = _mm_slli_si128(p0, 8);
          p0 = _mm_add_epi16(p0, s0);

          s0 = _mm_set1_epi16(img(y, x - 1));
          p0 = _mm_add_epi16(p0, s0);
